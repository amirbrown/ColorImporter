/*
 * Based on http://winterdom.com/2007/10/colorschemesinsql2005managementstudio 
 */
...
namespace Winterdom.Utilities.VSColorsToSql {
	class ColorImporter {
		private IDictionary<string, string> _src = new Dictionary<string, string>();
		private IDictionary<string, string> _dst = new Dictionary<string, string>();

		private IDictionary<string, string> _map = new Dictionary<string, string>();
		...
		public ColorImporter() {
			...
			_src.Add("2005", @"Software\Microsoft\VisualStudio\8.0\FontAndColors\{A27B4E24-A735-4D1D-B8E7-9716E1E3D8E0}");
			_src.Add("2008", @"Software\Microsoft\VisualStudio\9.0\FontAndColors\{A27B4E24-A735-4D1D-B8E7-9716E1E3D8E0}");
			_src.Add("2010", @"Software\Microsoft\VisualStudio\10.0\FontAndColors\{A27B4E24-A735-4D1D-B8E7-9716E1E3D8E0}");

			_dst.Add("2005", @"Software\Microsoft\Microsoft SQL Server\90\Tools\Shell\FontAndColors\{A27B4E24-A735-4D1D-B8E7-9716E1E3D8E0}");
			_dst.Add("2008", @"Software\Microsoft\Microsoft SQL Server\100\Tools\Shell\FontAndColors\{A27B4E24-A735-4D1D-B8E7-9716E1E3D8E0}");

			_ignorePrefixes = new string[] { "CSS ", "Disassembly ", "HTML ", "Refactoring ", "XML Doc " };
		}

		public void Copy(String sourceVersion, String destinationVersion) {
			RegistryKey src = Registry.CurrentUser.OpenSubKey(_src[sourceVersion]);
			RegistryKey dst = Registry.CurrentUser.OpenSubKey(_dst[destinationVersion], true);
		...
		private void CopyValue(string valname, RegistryKey src, RegistryKey dst) {
			if (!Ignore(valname)) {
				...
				if (valname.StartsWith("Identifier") && !valname.Contains("Background")) {
					dst.SetValue(valname.Replace("Identifier", "Plain Text"), src.GetValue(valname));
				}
			}
		}
